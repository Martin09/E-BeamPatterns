...
WROTE THIS ON THE TRAIN FROM ZURICH BECAUSE I COULDN'T UPDATE MY PROJECT FROM GITHUB TO PULL THE LATEST FILES
JUST WROTE THE SECTION WHICH IS SUPPOSED TO CREATE BRANCHES WITH VARYING TAPERING AT THE INTERSECTION...
WILL INTEGRATE THE TWO CODES WHEN I GET BACK TO THE OFFICE
...
wafer_r = 25e3
waferVer = '100 Membrane Branches v1.1'.format(int(wafer_r / 1000))
...
class Frame(Cell):
    ...
    def make_tapered_cross(self, nw1, nw2, tw1, tw2, l1, l2, tl1, tl2, layer):
        cross_cell = Cell(
            'Cross_NW{:.0f}_TW{:.0f}_L{:.0f}_TL{:.0f}xNW{:.0f}_TW{:.0f}_L{:.0f}_TL{:.0f}'.format(nw1 * 1000, tw1 * 1000,
                                                                                                 l1, tl1, nw2 * 1000,
                                                                                                 tw2 * 1000, l2, tl2))
        l1 += tw2
        l2 += tw1
    ...
    def make_tapered_cross_array(self, _pitches, spacing, _nom_widths, _taper_widths, _lengths, _taper_lengths,
                                 rot_angle, array_height, array_width,
        ...
        if not (type(_lengths) == list):
            _lengths = [_lengths]
        if not (type(_taper_lengths) == list):
            _taper_lengths = [_taper_lengths]
        if not (type(_nom_widths) == list):
            _nom_widths = [_nom_widths]
        if not (type(_taper_widths) == list):
            _taper_widths = [_taper_widths]

        _length = _lengths[0]
        _taper_length = _taper_lengths[0]
        _nom_width = _nom_widths[0]
        _taper_width = _taper_widths[0]
        layer = layers[0]
        ...
        cross = self.make_tapered_cross(_nom_width, _nom_width, _taper_width, _taper_width*10, _length, _length,
                                        _taper_length, _taper_length, layer)
...
smField1.make_tapered_cross_array(pitches[0], slitColumnSpacing, 0.050, 0.01, 10., 0.2, rotAngle, 100, 100, 30,
                                  l_smBeam)
...
lgField.add(smField1, origin=(0, 0))
...
lgField.add(centerAlignField, origin=(0., 0.))
