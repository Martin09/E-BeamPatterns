...
from core import (Cell, CellArray, GdsImport, Elements, Path)
from shapes import (Circle, Rectangle, Label, LineLabel)
...
def dashed_line(pt1, pt2, dashlength, width, layer):
    ...
    dline = Cell('DASHLINE')
    dline.add(lines)
    # dline.add(Path([pt1, pt2], width=width, layer=layer))  # TEMPORARY
    return dline
...
class Wafer_GridStyle(Cell):
    ...
    def add_wafer_outline(self, layers):
        ...
        for l in layers:
            ...
            circ = Circle((0, 0), 25e3, 100, layer=l)
            outline.add(circ)
    ...
    def _place_blocks(self, radius=None):
        """
        Create the list of valid block sites based on block size and wafer diam.
        """
        # Give the option to specify an apparent wafer size when placing blocks, allows for slight overlap of features
        # over the edge of the wafer by giving this function a slightly larger wafer radius
        if not radius:
            radius = self.wafer_r  # Otherwise, use the real wafer radius

        ind_max = np.floor(radius / self.block_size).astype(int)
...
class Block(Cell):
    ...
    def __init__(self, name, cell, size,
        ...
        rows = np.floor((size[1] - 2 * edge_gap) / spacing[1])

        origin = size / 2. - spacing * np.array([cols - 1, rows - 1]) / 2.
