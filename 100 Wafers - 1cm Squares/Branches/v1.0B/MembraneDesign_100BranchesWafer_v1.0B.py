...
class Frame(Cell):
    ...
    def make_tapered_cross(self, nw1, nw2, tw1, tw2, l1, l2, tl1, tl2, layer):
        cross_cell = Cell(
            'TaperedCross_NW{:.0f}_TW{:.0f}_L{:.0f}_TL{:.0f}xNW{:.0f}_TW{:.0f}_L{:.0f}_TL{:.0f}'.format(nw1 * 1000,
                                                                                                        tw1 * 1000,
                                                                                                        l1, tl1,
                                                                                                        nw2 * 1000,
                                                                                                        tw2 * 1000, l2,
        ...
        cross_pts = [(tw2 / 2., tw1 / 2.), (tl1, nw1 / 2.), (l1 / 2., nw1 / 2.),
                     (l1 / 2., -nw1 / 2.), (tl1, -nw1 / 2.), (tw2 / 2., -tw1 / 2.),
                     (nw2 / 2., -tl2 / 2.), (nw2 / 2., -l2 / 2.), (-nw2 / 2., -l2 / 2.),
                     (-nw2 / 2., -tl2 / 2.), (-tw2 / 2., -tw1 / 2.)]
    ...
    def make_tapered_cross_array(self, _nom_width, _taper_widths, _length, _taper_length, nx, ny,
                                 spacing_structs, spacing_arrays, rot_angle, layers):
        ...
        if type(_nom_width) == list:  # Only accept a single membrane width for now
            _nom_width = _nom_width[0]
        if type(_length) == list:  # Only accept a single membrane length for now
            _length = _length[0]
        if type(_taper_length) == list:  # Only accept a single taper length for now
        ...
        for j, tw_vert in enumerate(_taper_widths):
            for i, tw_horiz in enumerate(_taper_widths):
                # Define a single cross
                cross = self.make_tapered_cross(_nom_width, _nom_width, tw_horiz, tw_vert, _length,
                                                _length, _taper_length, _taper_length, layer)
                # Define the cross array
                cross_array = Cell("TaperedCrossArray_{:.0f}_{:.0f}".format(tw_horiz * 1000, tw_vert * 1000))
                slit_array = CellArray(cross, nx, ny, (_length + spacing_structs, _length + spacing_structs))
                slit_array.translate(
                    (-(nx - 1) * (_length + spacing_structs) / 2., (-(ny - 1) * (_length + spacing_structs) / 2.)))
                cross_array.add(slit_array)
        ...
        text_string = 'NomW {:.0f}, TL {:.0f}'.format(_nom_width * 1000, _taper_length * 1000)
        text = Label(text_string, 5, layer=layer)
        text.translate(tuple(np.array(-text.bounding_box.mean(0))))
        frame_lbl.add(text)
...
length = 15.
taper_length = [1., 0.5]
taper_widths = [0.01, 0.02, 0.03, 0.04]
pitch = 1.
spacing = 10.
...
smField1.make_tapered_cross_array(width, taper_widths, length, taper_length[0], 3., 3., 5., 20., 0, l_smBeam)

smField2 = Frame("SmallField2", (smFrameSize, smFrameSize), [])
smField2.make_align_markers(2., 20., (180., 180.), l_lgBeam, joy_markers=True)
smField2.make_tapered_cross_array(width, taper_widths, length, taper_length[1], 3., 3., 5., 20., 0, l_smBeam)

smField3 = Frame("SmallField3", (smFrameSize, smFrameSize), [])
smField3.make_align_markers(2., 20., (180., 180.), l_lgBeam, joy_markers=True)
smField3.make_tapered_cross_array(width, taper_widths, length, taper_length[0], 3., 3., 5., 20., 45., l_smBeam)

smField4 = Frame("SmallField4", (smFrameSize, smFrameSize), [])
smField4.make_align_markers(2., 20., (180., 180.), l_lgBeam, joy_markers=True)
smField4.make_tapered_cross_array(width, taper_widths, length, taper_length[1], 3., 3., 5., 20., 45., l_smBeam)
...
centerLeftAlignField.add(make_qp())
...
centerRightAlignField.add(make_qp(), rotation=45)
