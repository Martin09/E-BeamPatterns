...
WAFER_ID = 'XXXXXXXXX'  # CHANGE THIS FOR EACH DIFFERENT WAFER
PATTERN = 'SQBR2.0'
...
wafer_r = 25e3
waferVer = '100 Membrane Branches v2.0'.format(int(wafer_r / 1000))
...
class MBE100Wafer(Wafer_GridStyle):
    ...
    def __init__(self, name, cells=None):
        ...
        self.add_block_labels(l_lgBeam, unique_labels=True, load_ids=True)
    ...
    def add_block_labels(self, layers, unique_labels=False, save_ids=True, load_ids=True):
        ...
        if os.path.isfile(master_db):
            with open(master_db, 'r') as f:
                try:
                    db_IDs = json.load(f)
                    db_label_set = set([item for sublist in list(db_IDs.values()) for item in sublist])
