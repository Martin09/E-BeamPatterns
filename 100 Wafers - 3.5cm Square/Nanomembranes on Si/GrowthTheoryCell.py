...
from gdsCAD_045.shapes import Box, Rectangle, Label, Disk, RegPolygon
from gdsCAD_045.core import Cell, Boundary, CellArray, Layout, Path
import numpy as np
from templates import Wafer_TriangStyle, dashedLine
...
def make_rotating_slits(length, width, N, radius, layers, angleRef=False):
    ...
    for l in layers:
        ...
        if angleRef:
            ...
            dLine = dashedLine(pt1, pt2, 2, width, l)
            lineCell.add(line)
...
def make_theory_cell():
    ...
    TopCell.add(wheel2, origin=(0., -120.))
