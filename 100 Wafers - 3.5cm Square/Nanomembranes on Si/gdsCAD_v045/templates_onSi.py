...
import numpy as np
from gdsCAD_v045.core import Cell, CellArray, GdsImport, Elements, Boundary
from gdsCAD_v045.shapes import (Circle, Rectangle, Label)
from gdsCAD_v045.utils import rotate, translate
...
class Wafer_GridStyle(Cell):
    ...
    def add_dicing_crosses(self, l=0):
        """
        Create dicing crosses
        """
        ...
        self.add(am0.copy().translate(tuple(np.array(self.block_size) * [0, 1])))
        self.add(am0.copy().translate(tuple(np.array(self.block_size) * [0, -1])))
        self.add(am0.copy().translate(tuple(np.array(self.block_size) * [1, 0])))
        self.add(am0.copy().translate(tuple(np.array(self.block_size) * [-1, 0])))
        ...
        for (i, pt) in enumerate(self.block_pts):
            ...
            if isinstance(cell, Cell):
                ...
                try:
                    ...
                    spacing = am_size = np.array([10e3, 10e3])
