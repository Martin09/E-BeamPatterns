...
from gdsCAD_v045.core import Cell, GdsImport, Path, Boundary
from gdsCAD_v045.shapes import Circle, Label, LineLabel

#Helper function:
#Given two points from a line, returns a cell containing a dashed line connecting the two points
def dashedLine(pt1, pt2, dashlength, width, layer):
        line = LineString((pt1,pt2))
        dashpts = np.arange(0,line.length,dashlength).tolist()
        if len(dashpts)%2 == 1: #Odd number
            dashpts.append(line.length) #Add last point on line
        dashpts = map(line.interpolate,dashpts) #Interpolate points along this line to make dashes
        dashpts = [pt.xy for pt in dashpts]
        dashpts = np.reshape(dashpts,(-1,2,2))
        lines = [Path(list(linepts), width=width,layer=layer) for linepts in dashpts]
        dline = Cell('DASHLINE')
        dline.add(lines)
        return dline
...
#            centroidDist = np.array([cell.centroid.xy for cell in cells]).squeeze()
        ...
        block = translateshape(block, xoff=self.blockOffset[0], yoff=self.blockOffset[1]) #TODO: plot output not working properly because of this?
...
#                             facecolor='r',        
        ...
        for cell in cells:
            ...
            cell = translateshape(cell, xoff=self.blockOffset[0], yoff=self.blockOffset[1]) #TODO: plot output not working properly because of this?
    ...
    def add_blocks(self):
        ...
        layout = GdsImport('GrowthTheoryCell.gds',layers={1:2})
        self.cells.add(layout)        
        
        self.upCellLattice = self.getCellLattice(cellsize=2000)
        uplattice = self.upCellLattice
        self.downCellLattice = np.array(uplattice) * np.array([1, -1])
        downlattice = self.downCellLattice
        #Create a cell for both upward and downward facing triangular blocks
        upblock = Cell('upblock')
        for x, y in uplattice:
            upblock.add(self.cells, origin=(x, y))
        downblock = Cell('downblock')
        for x, y in downlattice:
            downblock.add(self.cells, origin=(x, y))
        #Take each point in block lattice and make a copy of the block in that location
        for x, y in self.upCenters:
            self.add(upblock, origin=(x, y))
        for x, y in self.downCenters:
            self.add(upblock, origin=(x, y), rotation=180)
        ...
        for i, pt in enumerate(cellLattice):
            ...
            for l in layers:
                ...
                if center:
                    ...
                    celllabelsUp.add(celllabel, origin=(0, -h / 2.*vOffsetFactor + np.mean(bbox, 0)[1])) #Bottom of cell                    
        for tri in self.upTris:
        ...
        for i, pt in enumerate(cellLattice):
            ...
            for l in layers:
                ...
                if center:
                    ...
                    celllabelsDown.add(celllabel, origin=(0, -h / 2.*vOffsetFactor + np.mean(bbox, 0)[1])) #Bottom of cell 
        for tri in self.downTris:
...
class Blocks(Cell):
    ...
    def __init__(self, sidelen, orient, center=[0., 0.]):
        self.center = center
        self.orient = orient
        self.sidelen = sidelen
        self.height = np.sqrt(3.) / 2. * sidelen
