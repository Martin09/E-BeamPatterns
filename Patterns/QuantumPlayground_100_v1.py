...
def makeShape_X(pitch, length, width, n_buffers, layer):
    ...
    xShape = Cell('XShape')
    xShape.add(long_slit, rotation=0)
    xShape.add(long_slit, rotation=90)
    ...
    xShape.add(allBuffers)
    xShape.add(allBuffers, rotation=90)
    xShape.add(allBuffers, rotation=180)
    xShape.add(allBuffers, rotation=270)
...
def makeShape_Star(pitch, length, width, n_buffers, layer):
    ...
    for i in range(n_buffers):
        smallBuffer.add(short_slit, rotation=0,
                        origin=((i + 1) * pitch * (1 + 2 * np.cos(np.deg2rad(45))), (i + 1) * pitch))
        smallBuffer.add(short_slit, rotation=45,
                        origin=((i + 1) * pitch * (1 + 2 * np.cos(np.deg2rad(45))), (i + 1) * pitch))
...
def makeShape_HashTag(hashtag_pitch, buffer_pitch, length, width, layer):
    ...
    :return: Cell with the desired pattern on it
    """
    dx_big = hashtag_pitch / 2.
    dy_big = hashtag_pitch / 2.
    dx_small = buffer_pitch
    ...
    hashtagShape.add(long_slit, origin=(0, -dy_big))
    hashtagShape.add(long_slit, origin=(0, dy_big))
    hashtagShape.add(long_slit, origin=(-hashtag_pitch / 2., 0), rotation=90)
    hashtagShape.add(long_slit, origin=(hashtag_pitch / 2., 0), rotation=90)
    ...
    shortline = Path([(-hashtag_pitch / 2. + buffer_pitch, 0), (hashtag_pitch / 2. - buffer_pitch, 0)], width=width,
                     layer=layer)
    short_slit = Cell('ShortSlit')
    short_slit.add(shortline)
    ...
    for i in range(n_buffers // 2):
        buffers.add(short_slit, origin=(0, dy_big - (i + 1) * dy_small))
        buffers.add(short_slit, origin=(0, -dy_big + (i + 1) * dy_small))
    ...
    len_buffer = hashtag_pitch - 2 * buffer_pitch
    bufferRow.add(buffers)
    bufferRow.add(buffers, origin=(hashtag_pitch / 2. + buffer_pitch + len_buffer / 2., 0))
    bufferRow.add(buffers, origin=(-hashtag_pitch / 2. - buffer_pitch - len_buffer / 2., 0))

    hashtagShape.add(bufferRow)
    hashtagShape.add(bufferRow, origin=(0, 2 * dy_big))
    hashtagShape.add(bufferRow, origin=(0, -2 * dy_big))
...
def makeShape_Window(frame_pitch, buffer_pitch, length, width, layer):
    ...
    windowShape.add(long_slit, origin=(0, -dy_big))
    windowShape.add(long_slit, origin=(0, dy_big))
    windowShape.add(long_slit, origin=(0, -3 * dy_big))
    windowShape.add(long_slit, origin=(0, 3 * dy_big))
    windowShape.add(long_slit, origin=(-frame_pitch / 2., 0), rotation=90)
    windowShape.add(long_slit, origin=(frame_pitch / 2., 0), rotation=90)
    windowShape.add(long_slit, origin=(-3 * frame_pitch / 2., 0), rotation=90)
    windowShape.add(long_slit, origin=(3 * frame_pitch / 2., 0), rotation=90)
    ...
    shortline = Path([(-frame_pitch / 2. + buffer_pitch, 0), (frame_pitch / 2. - buffer_pitch, 0)], width=width,
                     layer=layer)
    short_slit = Cell('ShortSlit')
    short_slit.add(shortline)
    ...
    bufferRow = Cell("BufferRow")
    len_buffer = frame_pitch - 2 * buffer_pitch
    v1 = (frame_pitch / 2. + buffer_pitch + len_buffer / 2., 0)
    bufferRow.add(buffers)
    bufferRow.add(buffers, origin=(v1[0], v1[1]))
    bufferRow.add(buffers, origin=(-v1[0], -v1[1]))
    bufferRow.add(buffers, origin=(-2 * v1[0], -2 * v1[1]))
    bufferRow.add(buffers, origin=(2 * v1[0], 2 * v1[1]))

    v2 = (2 * dx_big, 2 * dy_big)
    windowShape.add(bufferRow)
    windowShape.add(bufferRow, origin=(0, v2[1]))
    windowShape.add(bufferRow, origin=(0, -v2[1]))
    windowShape.add(bufferRow, origin=(0, 2 * v2[1]))
    windowShape.add(bufferRow, origin=(0, -2 * v2[1]))

    return windowShape


def makeShape_Square(pitch, length, width, contactlength, n_outer_buf, layer):
    """
    ...
    """
    squareshape = Cell("SquareShape")

    length = length * np.sqrt(3.) / 2.
    pt1 = (-length / 2., -length / 2.)
    pt2 = (length / 2., -length / 2.)
    pt3 = (-length / 2., length / 2.)
    pt4 = (length / 2., length / 2.)
    square_arm = Cell('SquareArms')

    # Make the main square with contact area
    long_line_cell = Cell('LongLineCell')
    longline = Path([(-length / 2. - contactlength, 0), (length / 2., 0)], width=width, layer=layer)
    long_line_cell.add(longline)
    square_arm.add(long_line_cell, origin=(0, length / 2.), rotation=0)

    short_line_cell = Cell('ShortLineCell')
    shortline = Path([(-length / 2., 0), (length / 2., 0)], width=width, layer=layer)
    short_line_cell.add(shortline)
    square_arm.add(short_line_cell, origin=(length / 2., 0), rotation=90)

    n_inner_buf = int(length / 2. / pitch)

    # Make the inner buffer squares
    for i in range(1, n_inner_buf + 1):
        buff_line_cell = Cell('BuffLine')
        slit_len = length - 2 * pitch * i
        buff_line = Path([(-slit_len / 2., 0), (slit_len / 2., 0)], width=width, layer=layer)
        buff_line_cell.add(buff_line)
        square_arm.add(buff_line_cell, origin=(0, length / 2. - pitch * i))
        square_arm.add(buff_line_cell, origin=(length / 2. - pitch * i, 0), rotation=90)

    # Make the outer buffer membranes
    for i in range(1, n_outer_buf + 1):
        buff_len = length + 2 * pitch * i
        long_buff_cell = Cell('LongBuffLine')
        long_buff = Path([(-buff_len / 2. - contactlength, 0), (buff_len / 2., 0)], width=width, layer=layer)
        long_buff_cell.add(long_buff)
        square_arm.add(long_buff_cell, origin=(0, length / 2. + i * pitch))

        short_buff_cell = Cell('ShortBuffLine')
        short_buff = Path([(-length / 2. + pitch, 0), (buff_len / 2., 0)], width=width, layer=layer)
        short_buff_cell.add(short_buff)
        square_arm.add(short_buff_cell, origin=(length / 2. + i * pitch, 0), rotation=90)

    square_arm_offcenter = Cell('SquareArmOffCenter')
    square_arm_offcenter.add(square_arm)

    squareshape.add(square_arm_offcenter, rotation=0)
    # squareshape.add(square_arm_offcenter, rotation=90)
    squareshape.add(square_arm_offcenter, rotation=180)
    # squareshape.add(square_arm_offcenter, rotation=270)
...
def makeShape_ABHexagon(pitch, length, width, contactlength, n_outer_buf, layer):
    ...
    line = Cell('HexagonShape')

    height = length * np.sqrt(2)
    pt1 = (-length / 2., 0)
    pt2 = (length / 2., 0)
    ...
    line_cell.add(line)
    shape_half.add(line_cell, origin=(length * (2 + np.sqrt(2)) / 4., length * np.sqrt(2) / 4.), rotation=-45)
    shape_half.add(line_cell, origin=(length * (2 + np.sqrt(2)) / 4., -length * np.sqrt(2) / 4.), rotation=45)
    shape_half.add(line_cell, origin=(0, height / 2.), rotation=0)

    contactline_cell = Cell('ContactLineCell')
    contactline = Path([(0, 0), (contactlength, 0)], width=width, layer=layer)
    contactline_cell.add(contactline)
    shape_half.add(contactline_cell, origin=(length / 2. + length * np.sqrt(2) / 2., 0), rotation=0)

    n_inner_buf = int(height / 2. / pitch) + 1
    tan_theta = length / 2. / (height / 2.)

    # Make the inner buffer hexagons
    for i in range(1, n_inner_buf):
        line_cell = Cell('LineCell')
        buff_height = height - 2 * i * pitch
        slit_len = buff_height * tan_theta
        line = Path([(-slit_len / 2., 0), (slit_len / 2., 0)], width=width, layer=layer)
        line_cell.add(line)
        shape_half.add(line_cell, origin=(slit_len * (2 + np.sqrt(2)) / 4., slit_len * np.sqrt(2) / 4.), rotation=-45)
        shape_half.add(line_cell, origin=(slit_len * (2 + np.sqrt(2)) / 4., -slit_len * np.sqrt(2) / 4.), rotation=45)
        shape_half.add(line_cell, origin=(0, buff_height / 2.), rotation=0)
    ...
    for i in range(1, n_outer_buf + 1):
        ...
        line = Path([(-slit_len / 2., 0), (slit_len / 2., 0)], width=width, layer=layer)
        line_cell.add(line)
        shortline = Path([(-slit_len / 2., 0), (slit_len / 2. - np.sqrt(2.) * pitch, 0)], width=width, layer=layer)
        shortline_cell = Cell('ShortLineBuffer')
        shortline_cell.add(shortline)

        shape_half.add(shortline_cell, origin=(slit_len * (2 + np.sqrt(2)) / 4., slit_len * np.sqrt(2) / 4.),
                       rotation=-45)
        shape_half.add(shortline_cell, origin=(slit_len * (2 + np.sqrt(2)) / 4., -slit_len * np.sqrt(2) / 4.),
                       rotation=45)
        shape_half.add(line_cell, origin=(0, buff_height / 2.), rotation=0)
...
def makeShape_ABOctagon(pitch, length, width, contactlength, n_outer_buf, layer):
    """
    Makes Aharonov Bohm hexagon shape
    :param pitch: pitch of nanomembranes
    :param length: hexagon side length
    ...
    :return: cell with triangle shape in it
    """
    octagonshape = Cell("OctagonShape")

    line = Cell('OctagonShape')

    height = length * (1 + np.sqrt(2))
    pt1 = (-length / 2., 0)
    pt2 = (length / 2., 0)

    shape_half = Cell('OctagonShapeHalf')
    ...
    line_cell.add(line)
    contactline_cell.add(contactline)
    shape_half.add(line_cell, origin=(height / 2., 0), rotation=90)
    shape_half.add(line_cell, origin=(length * (2 + np.sqrt(2)) / 4., length * (2 + np.sqrt(2)) / 4.), rotation=-45)
    shape_half.add(line_cell, origin=(length * (2 + np.sqrt(2)) / 4., -length * (2 + np.sqrt(2)) / 4.), rotation=45)
    shape_half.add(contactline_cell, origin=(0, height / 2.), rotation=0)
    ...
    for i in range(1, n_inner_buf):
        line_cell = Cell('LineCell')
        slit_len = length - 2 * i * pitch * np.tan(np.pi / 8.)
        buff_height = slit_len * (1 + np.sqrt(2))
        line = Path([(-slit_len / 2., 0), (slit_len / 2., 0)], width=width, layer=layer)
        line_cell.add(line)
        shape_half.add(line_cell, origin=(buff_height / 2., 0), rotation=90)
        shape_half.add(line_cell, origin=(slit_len * (2 + np.sqrt(2)) / 4., slit_len * (2 + np.sqrt(2)) / 4.),
                       rotation=-45)
        shape_half.add(line_cell, origin=(slit_len * (2 + np.sqrt(2)) / 4., -slit_len * (2 + np.sqrt(2)) / 4.),
                       rotation=45)
        shape_half.add(line_cell, origin=(0, buff_height / 2.), rotation=0)
    ...
    for i in range(1, n_outer_buf + 1):
        ...
        shape_half.add(line_cell, origin=(slit_len * (2 + np.sqrt(2)) / 4., -slit_len * (2 + np.sqrt(2)) / 4.),
                       rotation=45)
        shape_half.add(line_cell, origin=(buff_height / 2., 0), rotation=90)
        shape_half.add(shortline_cell, origin=(slit_len * (2 + np.sqrt(2)) / 4., slit_len * (2 + np.sqrt(2)) / 4.),
                       rotation=-45)
        shape_half.add(contactline_cell, origin=(0, buff_height / 2.))

    octagonshape.add(shape_half, rotation=0)
    octagonshape.add(shape_half, rotation=180)
...
def make_qp():
    ...
    for i, nm_width in enumerate(widths):
        ...
        cell_Square = makeShape_Square(1.0, 5, nm_width, 2.0, 3, l_smBeam)
        cell_SquareNoBuffer = makeShape_Square(10.0, 5, nm_width, 2.0, 0, l_smBeam)
        cell_AB_Hexagon = makeShape_ABHexagon(1.0, 3, nm_width, 2.0, 3, l_smBeam)
        cell_AB_HexagonNoBuffer = makeShape_ABHexagon(10.0, 3, nm_width, 2.0, 0, l_smBeam)
        cell_AB_Octagon = makeShape_ABOctagon(1.0, 2, nm_width, 2.0, 3, l_smBeam)
        cell_AB_OctagonNoBuffer = makeShape_ABOctagon(10.0, 2, nm_width, 2.0, 0, l_smBeam)

        QP = Cell('QuantumPlayground_W{:.0f}'.format(nm_width * 1000))
        QP.add(cell_AB_Hexagon, origin=(-30, 30))
        QP.add(cell_AB_HexagonNoBuffer, origin=(-10, 30))
        ...
        QP.add(cell_Square, origin=(-30, -30))
        QP.add(cell_SquareNoBuffer, origin=(-10, -30))
        QP.add(cell_AB_Octagon, origin=(10, 30))
        QP.add(cell_AB_OctagonNoBuffer, origin=(30, 30))
        ...
        cell_Square_sm = makeShape_Square(1.0, 1, nm_width, 1.0, 5, l_smBeam)
        cell_SquareNoBuffer_sm = makeShape_Square(10.0, 1, nm_width, 1.0, 0, l_smBeam)
        ...
        cell_AB_Octagon_sm = makeShape_ABOctagon(1.0, 1, nm_width, 1.0, 5, l_smBeam)
        cell_AB_OctagonNoBuffer_sm = makeShape_ABOctagon(10.0, 1, nm_width, 1.0, 0, l_smBeam)

        QP_sm = Cell('QuantumPlayground_W{:.0f}'.format(nm_width * 1000))
        QP_sm.add(cell_AB_Hexagon_sm, origin=(-30, 30))
        QP_sm.add(cell_AB_HexagonNoBuffer_sm, origin=(-10, 30))
        ...
        QP_sm.add(cell_Square_sm, origin=(-30, -30))
        QP_sm.add(cell_SquareNoBuffer_sm, origin=(-10, -30))
        QP_sm.add(cell_AB_Octagon_sm, origin=(10, 30))
        QP_sm.add(cell_AB_OctagonNoBuffer_sm, origin=(30, 30))
