...
from gdsCAD_py3.templates100 import dashed_line
...
def slit_elongation_array(pitches, spacing, widths, lengths, rot_angle, array_height, array_spacing, layers):
    ...
    for l in layers:
        ...
        slitarray.add(slits)
        text = Label('w/p\n%i/%i' % (width * 1000, pitch * 1000), 2, layer=l)
        lbl_vert_offset = 1.4
    ...
    for l in layers:
        ...
        for x_var in x_vars:
            ...
            for y_var in y_vars:
                ...
                text = Label('w/p/l\n%i/%i/%i' % (width * 1000, pitch * 1000, length * 1000), 2, layer=l)
...
if __name__ == "__main__":
    TopCell = make_theory_cell_4br()
    # Add the copied cell to a Layout and save
    layout = Layout('LIBRARY')
    layout.add(TopCell)
    layout.save('GrowthTheoryCell_4BD.gds')
