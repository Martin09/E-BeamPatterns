...
class Path(ElementBase):
    ...
    def to_gds(self, multiplier):
        ...
        data = struct.pack('>12H', 4, 0x0900, 6, 0x0D02, self.layer, 6, 0x0E02, self.datatype, 6, 0x2102, self.pathtype,
                           8)
        data += struct.pack('>HL2H', 0x0F03, int(round(self.width * multiplier)), 4 + 8 * gds_coordinates.shape[0],
                            0x1003)
...
class Text(ElementBase):
    ...
    def __init__(self, text, position, anchor='o', rotation=None, magnification=None, layer=None, datatype=None,
    ...
    def __str__(self):
        return "Text (\"{0}\", at ({1[0]}, {1[1]}), rotation {2}, magnification {3}, layer {4}, texttype {5})".format(
            self.text, self.points, self.rotation, self.magnification, self.layer, self.texttype)
    ...
    def to_gds(self, multiplier):
        ...
        return data + struct.pack('>2h2l2h', 12, 0x1003, int(round(self.points[0] * multiplier)),
                                  int(round(self.points[1] * multiplier)), 4 + len(text), 0x1906) + text.encode(
            'ascii') + struct.pack('>2h', 4, 0x1100)
...
class Elements(object):
    ...
    def __str__(self):
        return "Elements layer={}, datatype={} ({} polygons, {} vertices)".format(self.layer, self.datatype,
                                                                                  len(self.polygons), sum(
                [len(p.points) for p in self.polygons]))
...
class Layout(dict):
    ...
    def save(self, outfile):
        ...
        if len(self.name) % 2 != 0:
            ...
            name = self.name
        outfile.write(
            struct.pack('>19h', 6, 0x0002, 0x0258, 28, 0x0102, now.year, now.month, now.day, now.hour, now.minute,
                        now.second, now.year, now.month, now.day, now.hour, now.minute, now.second, 4 + len(name),
                        0x0206) + name.encode('ascii') + struct.pack('>2h', 20, 0x0305) + _eight_byte_real(
...
class Cell(object):
    ...
    def to_gds(self, multiplier, duplicates=[]):
        ...
        if len(name) % 2 != 0:
            name = name + '\0'
        data = struct.pack('>16h', 28, 0x0502, now.year, now.month, now.day, now.hour, now.minute, now.second, now.year,
                           now.month, now.day, now.hour, now.minute, now.second, 4 + len(name), 0x0606) + name.encode(
            'ascii')
        for element in self:
...
class ReferenceBase:
    ...
    def translate(self, displacement):
        ...
        """
        self.origin = np.array(self.origin) + np.array(displacement)
        return self
...
class CellReference(ReferenceBase):
    ...
    def __repr__(self):
        if isinstance(self.ref_cell, Cell):
            ...
            name = self.ref_cell
        return "CellReference(\"{0}\", ({1[0]}, {1[1]}), {2}, {3}, {4})".format(name, self.origin, self.rotation,
...
class CellArray(ReferenceBase):
    ...
    def __init__(self, ref_cell, cols, rows, spacing, origin=(0, 0), rotation=None, magnification=None,
                 x_reflection=False):
        ReferenceBase.__init__(self)
    ...
    def __str__(self):
        ...
        return "CellArray (\"{0}\", {1} x {2}, at ({3[0]}, {3[1]}), spacing {4[0]} x {4[1]}, rotation {5}, magnification {6}, reflection {7})".format(
            name, self.cols, self.rows, self.origin, self.spacing, self.rotation, self.magnification, self.x_reflection)
    ...
    def __repr__(self):
        if isinstance(self.ref_cell, Cell):
            ...
            name = self.ref_cell
        return "CellArray(\"{0}\", {1}, {2}, ({4[0]}, {4[1]}), ({3[0]}, {3[1]}), {5}, {6}, {7})".format(name, self.cols,
                                                                                                        self.rows,
                                                                                                        self.origin,
                                                                                                        self.spacing,
                                                                                                        self.rotation,
    ...
    def to_gds(self, multiplier, duplicates=[]):
        ...
        if not (self.rotation is None) or not (self.magnification is None) or self.x_reflection:
            ...
            if not (self.rotation is None):
                ...
                values += struct.pack('>2h', 12, 0x1C05) + _eight_byte_real(self.rotation)
            data += struct.pack('>2hH', 6, 0x1A01, word) + values
        return data + struct.pack('>6h6l2h', 8, 0x1302, self.cols, self.rows, 28, 0x1003,
...
def GdsImport(infile, rename={}, layers={}, datatypes={}, verbose=True):
    ...
    _record_name = (
    'HEADER', 'BGNLIB', 'LIBNAME', 'UNITS', 'ENDLIB', 'BGNSTR', 'STRNAME', 'ENDSTR', 'BOUNDARY', 'PATH', 'SREF', 'AREF',
    'TEXT', 'LAYER', 'DATATYPE', 'WIDTH', 'XY', 'ENDEL', 'SNAME', 'COLROW', 'TEXTNODE', 'NODE', 'TEXTTYPE',
    'PRESENTATION', 'SPACING', 'STRING', 'STRANS', 'MAG', 'ANGLE', 'UINTEGER', 'USTRING', 'REFLIBS', 'FONTS',
    'PATHTYPE', 'GENERATIONS', 'ATTRTABLE', 'STYPTABLE', 'STRTYPE', 'ELFLAGS', 'ELKEY', 'LINKTYPE', 'LINKKEYS',
    'NODETYPE', 'PROPATTR', 'PROPVALUE', 'BOX', 'BOXTYPE', 'PLEX', 'BGNEXTN', 'ENDTEXTN', 'TAPENUM', 'TAPECODE',
    ...
    while record is not None:
        ...
        ## WIDTH
            ...
            if record[1][0] < 0 and record[0] not in emitted_warnings:
                warnings.warn(
                    "[GDSPY] Paths with absolute width value are not supported. Scaling these paths will also scale their width.",
                    stacklevel=2)
        ...
        ## Not supported
            ...
            warnings.warn("[GDSPY] Record type {0} not supported by gds_import.".format(_record_name[record[0]]),
                          stacklevel=2)
            emitted_warnings.append(record[0])
        record = _read_record(infile)
    if close:
